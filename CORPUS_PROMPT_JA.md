...
9. キャラクター分散仕様:
   ...
   - 3〜6種類のキャラクター文体を混在させる（例: 執事風、先輩風、先生風、友人風、クール系）
   - 執事風は任意（0文でも可）で、必須ではない
   - 全200文のうち、キャラクター文体は40〜110文
   - 各50文ブロック内で、任意の単一文体が30文を超えない
   - 同一キャラクターが6文以上連続しない
...
## 監査CSVの値定義
- `character_style`: `none` または文体名（例: `butler`, `senpai`, `teacher`, `friend`, `cool`）
